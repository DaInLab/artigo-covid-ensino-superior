...
Como pode ser observado na figura do Gráfico 1, a pesquisa contou com a participação de `r total_casos` integrantes, sendo que `r pct_genero[3]` se declaram do gênero masculino, `r pct_genero[1]` feminino e `r pct_genero[2]` LGBT. A amostra foi composta ainda por `r pct_ies[11]` de alunos da UNESP e `r sem_unesp_ies` de demais instituições. 
...
Com relação questão da vacinação contra a COVID-19, nesta amostra, a maioria dos estudantes (`r paste0(total_vacina, "%")`foi vacinada, como é possível observar no Gráfico 7, somando-se os dados dos que foram imunizados com duas doses ou dose única e os com duas doses ou dose única e reforço (Gráfico 7).  
...
Outro fator importante a ser levado em consideração foi a questão de locomoção - impedidas pelas restrições e *lockdown* devido a pandemia - e o fato de que a maior parte dos respondentes da amostra não são residentes da cidade em que estudam, pois apenas `r casos_local_estudante[n-1]` (`r pct_casos_local_estudante[n-1]`) dos entrevistados residem na mesma cidade da universidade e/ou faculdade e a maioria, `r pct_casos_local_estudante[n]` (`r casos_local_estudante[n-1]` casos) em outras cidades (Gráfico 9).  
...
Destes, 90% estavam na sua residência permanente durante a pandemia e 10% não.  
...
pie(casos_migrou_virtual,
    ...
    main = "Gráfico 11: IES migrou para virtual")
```
