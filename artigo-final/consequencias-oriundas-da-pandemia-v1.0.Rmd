...
```{r setup_geral, include=FALSE}
options(tinytex.verbose = TRUE)

knitr::opts_chunk$set(echo = TRUE) # Opções de compilação inicial: Mostrar código fonte no texto
knitr::opts_chunk$set(warning = FALSE, message = FALSE) # Opções de compilação: NÃO mostrar mensagens de erro e de aviso no texto

#Carregando os pacotes necessários ao programa
...
library(tm)
if(!"magrittr" %in% installed.packages()) install.packages("magrittr") # Para utilizar o comando "pipe" (%>%) ou operador "tee pipe" (%T>%)
library(magrittr)
...
if (get_os() == "windows"){
  knitr::opts_knit$set(root.dir = "D:/GitHub/artigo-covid-ensino-superior/") # Deve ser modificado de acordo com o diretório de trabalho no computador 
}
if (get_os() == "osx"){
     knitr::opts_knit$set(root.dir = "/Volumes/Extension/GitHub/artigo-covid-ensino-superior") # Deve ser modificado de acordo com o diretório de trabalho no computador 
}
...
# dados do dataset atualizados em 30/05/2023.
...
# Deve ser modificado de acordo com o diretório de trabalho no computador
if (get_os() == "windows") dbf.xlsx <- read_excel("D:/GitHub/artigo-covid-ensino-superior/dados/COVID-19-E-IES-2023.xlsx")
if (get_os() == "osx") dbf.xlsx <- read_excel("/Volumes/Extension/GitHub/artigo-covid-ensino-superior/dados/COVID-19-E-IES-2023.xlsx")
```
...
# Sumário do data set
# número total de observações no data frame
total_casos <- nrow(dbf.xlsx)

#Quantidade de respondentes por sexo
casos_genero <- table(dbf.xlsx$genero)
pct_genero <- paste(round(unname(casos_genero) / sum(unname(casos_genero)) * 100), "%")

# Quantidade de respondentes por Instituição de Ensino Superior
...
pct_ies <- paste0(pct_ies, "%")
```

Como pode ser observado na figura do Gráfico 1, a pesquisa contou com a participação de `r total_casos` integrantes, sendo que `r pct_genero[3]` se declaram do gênero masculino, `r pct_genero[1]` feminino e `r pct_genero[2]` LGBT. A amostra foi composta ainda por `r pct_ies[11]` de alunos da UNESP e `r sem_unesp_ies` de outras instituições. 
...
```{r, grafico-2-pre-calc, echo=FALSE}
## Calculando valores para o Gráfico 2 - Faixa Etária dos respondentes
...
# Encontrando as outras faixas etárias representativas abordagem
#Cálculo da porcentagem das faixas etárias
pct_idade4 <- paste0(round(unname(sort(casos_idade_concat)) / sum(unname(sort(casos_idade_concat))) * 100,0), "%")
n <- length(casos_idade_concat)
#sort(casos_idade_concat,partial=n-2)[n-2] # terceiro maior
#names(sort(casos_idade_concat,partial=n-2)[n-2]) # quantidade do terceiro maior
#pct_idade4[n-2] # posição na array de percentual do terceiro maior
```

## Perfil dos entrevistados
Para o perfil dos entrevistados foram analisadas as respostas obtidas pelo questionário, no período disponível. A faixa etária com maior número de entrevistados, correspondendo a `r pct_idade3[unname(which.max(casos_idade_concat))]`, é a faixa etária `r names(which.max(casos_idade_concat))`. A segunda faixa etária com maior número de respondentes nesta amostra é a faixa etária correspondente a `r names(which.max(casos_idade_concat[casos_idade_concat!=max(casos_idade_concat)]))` com `r pct_idade3[unname(which.max(casos_idade_concat[casos_idade_concat!=max(casos_idade_concat)]))]`.  
A faixa de `r names(sort(casos_idade_concat,partial=n-2)[n-2])` obteve `r pct_idade4[n-2]`, a faixa de `r names(sort(casos_idade_concat,partial=n-3)[n-3])` obteve `r pct_idade4[n-3]`e a faixa de `r `names(sort(casos_idade_concat,partial=n-4)[n-4])` obteve `r pct_idade4[n-4]`, como é possível observar na figura do Gráfico 2.  
...
```{r, grafico-3-pre-calc, echo=FALSE}
...
Quanto a situação civil dos participantes, são `r paste0(tolower(names(sort(casos_conjugal,partial=n-0)[n-0])), "s")`, `r pct_conjugal[n-0]`,  são `r paste0(tolower(names(sort(casos_conjugal,partial=n-1)[n-1])), "s")`, `r pct_conjugal[n-1]`, `r pct_conjugal[n-2]` estão em `r tolower(names(sort(casos_conjugal,partial=n-2)[n-2]))`, `r pct_conjugal[n-3]` são `r paste0(tolower(names(sort(casos_conjugal,partial=n-3)[n-3])), "s")` e `r pct_conjugal[n-4]` é `r tolower(names(sort(casos_conjugal,partial=n-4)[n-4]))`.  
...
pie(sort(casos_conjugal),
    ...
    main = "Gráfico 3: Quantidade de respondentes por estado civil")
```

```{r, grafico-4-pre-calc, echo=FALSE}
# Dados do Gráfico 4 Quantidade de respondentes por nível de ensino
casos_ensino<- table(dbf.xlsx$nivel_ensino, exclude = NULL)
pct_ensino <- paste(round(unname(sort(casos_ensino)) / sum(unname(sort(casos_ensino))) * 100), "%")
# Determinando o maior número de casos de nível de ensino
n <- length(casos_ensino)
```

Ainda caracterizando o perfil dos entrevistados, na figura do Gráfico 4 foi constatado que o nível de escolaridade predominante da amostra é a `r names(sort(sort(casos_ensino,partial=n-0)[n-0]))`, com `r unname(sort(sort(casos_ensino,partial=n-0)[n-0]))` respondentes (`r pct_ensino[n-0]`), seguido pelo `r names(sort(sort(casos_ensino,partial=n-1)[n-1]))` com `r unname(sort(sort(casos_ensino,partial=n-1)[n-1]))` casos (`r pct_ensino[n-1]`) e `r names(sort(sort(casos_ensino,partial=n-2)[n-2]))` com `r unname(sort(sort(casos_ensino,partial=n-2)[n-2]))` respondentes (`r pct_ensino[n-2]`).  

```{r, grafico-4, echo=FALSE}
# Gráfico 4: Quantidade de respondentes por nível de ensino
# Gráfico tipo "pizza"
pie(sort(casos_ensino)[(n-2):n],
    edges = 200, radius = 0.8,
    clockwise = F,
    density = NULL, angle = 90, col = c("red", "orange", "yellow", "green", "black", "white"),
    labels = paste(names(sort(casos_ensino)[(n-2):n]), "-", pct_ensino[(n-2):n]),
    main = "Gráfico 4: Quantidade de respondentes por nível de ensino")
```

Os demais níveis de formação, tais como `r names(sort(casos_ensino,partial=n-3)[n-3])`, `r names(sort(casos_ensino,partial=n-4)[n-4])` e `r names(sort(casos_ensino,partial=n-5)[n-5])` constam com `r unname(sort(casos_ensino,partial=n-5)[n-5])` entrevistado cada nível, ou apenas  `r pct_ensino[n-5]` da amostra. 

```{r, grafico-5-pre-calc, echo=FALSE}
# Dados do Gráfico 5: Quantidade de respondentes por tipo de mantenedora de instituição de ensino
...
n <- length(casos_tipo_ies)
```

Do total de respondentes da pesquisa, a grande maioria (`r pct_tipo_ies[n-0]`) são de instituição `r names(sort(sort(casos_tipo_ies,partial=n-0)[n-0]))`, `r pct_tipo_ies[n-1]` de `r names(sort(sort(casos_tipo_ies,partial=n-1)[n-1]))` (Gráfico 5). 
...
```{r, grafico-6-pre-calc, echo=FALSE}
# Dados do Gráfico 6 do tipo barra sitação empregatícia
...
Ao analisar as condições financeiras dos participantes por meio do Gráfico 6, foi possível identificar que, do total de participantes da amostra, `r pct_emprego[n-0]` são `r paste0(tolower(names(sort(sort(casos_emprego,partial=n-0)[n-0]))), "s")`, `r pct_emprego[n-1]` estão `r paste0(tolower(names(sort(sort(casos_emprego,partial=n-1)[n-1]))), "s")`, `r pct_emprego[n-2]` são `r paste0(tolower(names(sort(sort(casos_emprego,partial=n-2)[n-2]))), "s")` e `r pct_emprego[n-3]` são `r paste0(tolower(names(sort(sort(casos_emprego,partial=n-3)[n-3]))), "s")`.

```{r, grafico-6, echo=FALSE}
# Gráfico 6 do tipo barra sitação empregatícia
graph.casos_emprego <- barplot(sort(casos_emprego), 
                              main = "Gráfico 6: Quantidade de respondentes por sitação empregatícia",
...
Ainda com relação ao vículo empregatício da amostra, pode-se também pobservar na figura do Gráfico 6 que `r pct_emprego[n-4]` estão `r paste0(tolower(names(sort(sort(casos_emprego,partial=n-4)[n-4]))), "s")`, `r paste0(tolower(names(sort(sort(casos_emprego,partial=n-5)[n-5]))), "s")` representam `r pct_emprego[n-5]` dos casos e `r paste0(tolower(names(sort(sort(casos_emprego,partial=n-6)[n-6]))), "s")` e  `r paste0(tolower(names(sort(sort(casos_emprego,partial=n-7)[n-7]))), "s")` são apenas `r pct_emprego[n-7]` dos respondentes.

```{r, grafico-7-prep-calc, echo=FALSE}
# Dados do Gráfico 7: Foi vacinado?
...
Com relação questão da vacinação contra a COVID-19, nesta amostra, a maioria dos estudantes (`r paste0(total_vacina, "%")`foi vacinada, como é possível observar no Gráfico 7, somando-se os dados dos que foram imunizados com duas doses ou dose única e os com duas doses ou dose única e reforço.  

```{r, grafico-7, echo=FALSE}
# Dados do Gráfico 7: Foi vacinado?
# Gráfico tipo "pizza"
pie(casos_vacinado,
    edges = 200, radius = 0.8,
...
```{r, grafico-9-prep-calc, echo=FALSE}
# Dados do Gráfico 9 Local de residência do aluno
...
Outro fator importante a ser levado em consideração foi a questão de locomoção - impedidas pelas restrições e *lockdown* devido a pandemia - e o fato de que a maior parte dos respondentes da amostra não são residentes da cidade em que estudam, pois apenas `r casos_local_estudante[n-1]` (`r pct_casos_local_estudante[n-1]`) dos entrevistados residem na mesma cidade da universidade e/ou faculdade e a maioria, `r pct_casos_local_estudante[n]` (`r unname(casos_local_estudante[n-1])` casos) em outras cidades (Gráfico 9).  

```{r, grafico-9, echo=FALSE}
#Gráfico 9: Procedência do aluno
...
```{r, grafico-10-prep-calc, echo=FALSE}
...
Conforme demonstrado no Gráfico 10, dos alunos respondentes, `r pct_aluno_mora_com[n]` moravam com a `r tolower(names(casos_aluno_mora_com[n]))`, seguido de `r pct_aluno_mora_com[n-1]` morando `r tolower(names(casos_aluno_mora_com[n-1]))` e, em último ficaram os que residiam com `r tolower(names(casos_aluno_mora_com[n-2]))` (`r pct_aluno_mora_com[n-2]`).  
...
pie(casos_aluno_mora_com,
    ...
    main = "Gráfico 10: Tipo de moradia do aluno")
```
    
Assim, podemos observar no Gráfico 10, que, do toral de respondentes `r paste0(as.numeric(substr(pct_aluno_mora_com[n], 1,2)) + as.numeric(substr(pct_aluno_mora_com[n-1], 1,2)), "%")` estavam na sua residência permanente durante a pandemia e `r paste0(as.numeric(substr(pct_aluno_mora_com[n-2], 1,2)), "%")` não.  

## Decisões tomadas pelas IES, acesso à sua infraestrutura e aos docentes  

```{r, grafico-11-pre-proc, echo=FALSE}
# Dados do Gráfico 11
...
Sobre a decisão de fechar o campus para evitar a circulação de pessoas e, consequentemente, evitar a circulação do vírus entre a comunidade acadêmica de nível superior, na figura do Gráfico 11 nota-se que `r pct_fechar_ies[n]` dos pesquisados consideram as decisões tomadas pela IES `r tolower(names(casos_fechar_ies[n]))`, enquanto `r pct_fechar_ies[n-1]` consideram a tomada de decisão `r tolower(names(casos_fechar_ies[n-1]))` e `r pct_fechar_ies[n-2]` definem que as IES decidiram `r tolower(names(casos_fechar_ies[n-2]))`.
...
```{r, grafico-12-pre-proc, echo=FALSE}
# Dados do Gráfico 12
...
No que diz respeito à investigação se a Instituição de Nível Superior (IES) durante a pandemia transferiu todas suas atividades presenciais para o ensino remoto, apenas `r pct_migrou_virtual[n-1]` dos alunos da amostra informaram que sua instituição não havia migrado para as aulas virtuais durante a pandemia. Constata-se, entretanto, que `r pct_migrou_virtual[n]` das IES da amostra se adaptaram de alguma forma para educação remota, como mostra o Grafico 12. 

```{r, grafico-12, echo=FALSE}

#Gráfico 12: IES migrou virtual
# Gráfico tipo "pizza"
pie(casos_migrou_virtual,
    ...
    main = "Gráfico 12: IES migrou para virtual")
```
```{r, grafico-13-pre-proc, echo=FALSE}
# Dados do Gráfico 13
...
Já no tocante as formas de acesso aos recursos de infraestrutura oferecidos pela instituição (biblioteca, coordenação, orientação de assuntos acadêmicos, matrícula etc.) o Gráfico 13 mostra que, na opinião de `r pct_acesso_infra_ies[n]` dos estudantes, o acesso aos recursos da IES `r tolower(names(casos_acesso_infra_ies[n]))` durante a pandemia do COVID-19. Já `r pct_acesso_infra_ies[n-1]` alegam que `r tolower(names(casos_acesso_infra_ies[n-1]))`. 
...
pie(casos_acesso_infra_ies,
    ...
    main = "Gráfico 13: Acesso à infra-estrutura da IES")
```

Os dados no Gráfico 13 indicam ainda que `r pct_acesso_infra_ies[n-2]` alegam que o acesso à infra-estrutura da IES `r tolower(names(casos_acesso_infra_ies[n-2]))`, e, finalmente, `r pct_acesso_infra_ies[n-3]` dos respondentes `r tolower(names(casos_acesso_infra_ies[n-3]))`. 

```{r, grafico-14-pre-proc, echo=FALSE}
# Dados do Gráfico 14
...
Ao abordar o acesso ao corpo docente das IES durante a pandemia (Gráfico 14), a maioria dos respondentes sentiu que o acesso aos professores `r tolower(names(casos_acesso_professores[n]))` (`r pct_acesso_infra_ies[n]`) comparado a antes da pandemia, seguido por aqueles que acharam que o contato `r tolower(names(casos_acesso_professores[n-1]))` (`r pct_acesso_infra_ies[n-1]`).  
Para uma parcela menor (`r pct_acesso_infra_ies[n-2]`) a disponibilidade dos docentes `r tolower(names(casos_acesso_professores[n-2]))` durante a pandemia, outros `r pct_acesso_infra_ies[n-3]` julgam que o acesso aos docentes `r tolower(names(casos_acesso_professores[n-3]))` durante a pandemia e apenas `r pct_acesso_infra_ies[n-4]` não souberam responder.

```{r, grafico-14, echo=FALSE}
#Gráfico 14: Acesso aos professores na pandemia
# Gráfico tipo "pizza"
pie(casos_acesso_professores,
    ...
    main = "Gráfico 14: Acesso aos professores na pandemia")
```
```{r, grafico-15-pre-proc, echo=FALSE}
# Dados do Gráfico 15
...
Quando questionados sobre a forma como as aulas foram ministradas durante a pandemia, de acordo com figura do Gráfico 15, `r pct_aulas[n]` dos participantes opinaram que `r tolower(names(casos_aulas[n]))`. Por outro lado, `r pct_aulas[n-1]` afirmaram que a forma de ministrar as aulas no estilo remoto `r tolower(names(casos_aulas[n-1]))`, `r pct_aulas[n-2]` alegaram que as aulas `r tolower(names(casos_aulas[n-2]))` e `r pct_aulas[n-3]` `r tolower(names(casos_aulas[n-3]))`. 
...
pie(casos_aulas,
    ...
    main = "Gráfico 15: Modo de lecionar durante a pandemia")
```

É interessante observar na figura do Gráfico 15 que, apesar de uma certa piora no acesso à infraestrutura e professores, grande parte dos estudantes universitários que participaram da pesquisa não se sentiu afetado quanto à maneira como as aulas foram ministradas.

```{r, grafico-16-pre-proc, echo=FALSE}
# Dados do Gráfico 16
...
Após a vacinação e maior controle da pandemia, com dados coletados do início da pandemia, em 2020 até o encerramento desta orimeira fase da coleta, em 2022, no Gráfico 16 observa-se que `r pct_ies_reinicio[n]` dos entrevistados afirmavam que as universidades já haviam retomado as atividades de forma presencial nas IES e `r pct_ies_reinicio[n-1]` haviam retornado em parte. 
...
pie(casos_ies_reinicio,
    ...
    main = "Gráfico 16: IES reiniciou atividades presenciais")
```
...
```{r, grafico-17-pre-proc, echo=FALSE}
# Dados do Gráfico 17
...
De acordo com o Gráfico 17, `r pct_acesso_internet[n]` dos respondentes afirmaram o acesso à internet que `r tolower(names(casos_acesso_internet[n]))`, para `r pct_acesso_internet[n-1]` o acesso aos serviços foi `r tolower(names(casos_acesso_internet[n-1]))` e `r pct_acesso_internet[n-2]` opinaram que os serviços ficaram `r tolower(names(casos_acesso_internet[n-2]))`.  
De modo geral, para os estudantes pesquisados o acesso a internet não mudou.

```{r, grafico-17, echo=FALSE}
#Gráfico 17: Acesso aos serviços de internet
# Gráfico tipo "pizza"
pie(casos_acesso_internet,
    ...
    main = "Gráfico 17: Acesso aos serviços de internet")
```

```{r, grafico-18-pre-proc, echo=FALSE}
# Dados do Gráfico 18
...
No que concerne à mudança para o ensino virtual remoto e os possíveis impactos nodesempenho escolar do aluno, dada a mudança na forma de ensino, os dados do Gráfico 18 mostram que `r pct_performance[n]` dos participantes considera que seu desempenho escolar foi `r tolower(names(casos_performance[n]))`, `r pct_performance[n-1]` consideram que o seu desempenho `r tolower(names(casos_performance[n-1]))`, e `r pct_performance[n-2]` opinaram que seu rendimento escolar `r tolower(names(casos_performance[n-2]))` com o ensino remoto, se comparado ao ensino presencial.

```{r, grafico-18, echo=FALSE}
#Gráfico 18: Desempenho escolar
# Gráfico tipo "pizza"
pie(casos_performance,
    ...
    main = "Gráfico 18: Desempenho escolar na pandemia")
```

Observa-se nos resultados compilados no no Gráfico 18, que quanto a este quesito, as opiniões ficaram bastante divididas e apenas `r pct_performance[n-2]` não souberam opinar.  
Com as restrições sociais introduzidas durante o período de isolamento social, é importante também buscar entender como a pandemia dificultou de forma abrangente a vida dos docentes. 

```{r, grafico-19-pre-proc, echo=FALSE}
# Dados do Gráfico 19
...
Quando questionados diretamente quais os principais fatos vivenciados durante o período agudo da pandemia que os afetaram com intensidade, os alunos, de acordo com os dados compilados e mostrados no Gráfico 19, responderam que, que pelas orientações de distanciamento e fechamento dos serviços, tiveram `r tolower(names(casos_aluno_vivenciou[n]))` (`r pct_aluno_vivenciou[n]`), além de `r tolower(names(casos_aluno_vivenciou[n-2]))` (`r pct_aluno_vivenciou[n-2]`) e necessitaram de `r tolower(names(casos_aluno_vivenciou[n-3]))` (`r pct_aluno_vivenciou[n-3]`).    
Entretanto, para `r pct_aluno_vivenciou[n-1]` dos respondentes, tais atribulações não tiveram impacto ou não souberam ou quiseram responder.  

```{r, grafico-19, echo=FALSE}
#Gráfico 19: Dificuldades vividas pelo aluno
# Gráfico tipo "pizza"
pie(casos_aluno_vivenciou,
    ...
    main = "Gráfico 19: Dificuldades vividas pelo aluno")
```

Além das dificuldades apontadas nos dados do Gráfico 19, dada as restrições sociais a vida financeira dos discentes também foi afetada de formaa importante. 

```{r, grafico-20-pre-proc, echo=FALSE}
# Dados do Gráfico 20
...
Os dados compilados no Gráfico 20 ilustram a percepção dos alunos da amostra em relação aos seus gastos durante o a pandemia onde `r pct_despesas[n]` alegam que as despesas `r tolower(names(casos_despesas[n]))`, `r pct_despesas[n-1]` opinaram que suas despesas `r tolower(names(casos_despesas[n-1]))` enquanto que `r pct_despesas[n-2]` consideraram que estas `r tolower(names(casos_despesas[n-2]))`.  
...
pie(casos_despesas,
    ...
    main = "Gráfico 20: Gastos durante a pandemia")
```

```{r, grafico-21-pre-proc, echo=FALSE}
# Dados do Gráfico 21
casos_renda <- sort(table(dbf.xlsx$renda_financeira, exclude = NULL))
...
Nos dados do Gráfico 20, observamos que grande parte dos entrevistados sofreram com aumento de despesas durante a pandemia.  
Outro informação interessante a ser observada nos dados compilados, é a questão do poder de compra dos alunos. No figura do Gráfico 21, `r pct_renda[n]` dos discentes participantes consideraram que a sua renda continuou relativamente a mesma durante a pandemia 
Já `r pct_renda[n-1]` dos alunos consideram que sua renda diminuiu. Apenas `r pct_renda[n-1]` consideraram que a sua renda aumentou.
...
```{r, grafico-22-pre-proc, echo=FALSE} 
# Dados do Gráfico 22
...
Ainda que muitas universidades públicas ofereçam auxílio financeiro para que os estudantes possam residir no campus (ou próximo de) para estudar, de acordo com os dados do Gráfico 22 observa-se que durante o período de pandemia, `r pct_ajuda[n]` dos entrevistados não fruíram nenhum respaldo por parte da instituição educacional ou de outra organização. 

```{r, grafico-22, echo=FALSE}
#Gráfico 22: Recebeu auxílio financeiro da IES
# Gráfico tipo "pizza"
pie(casos_ajuda,
    edges = 200, radius = 0.8,
    clockwise = F,
    ...
    main = "Gráfico 22: Recebeu auxílio financeiro da IES")
```


```{r, grafico-23-pre-proc, echo=FALSE}
# Dados do Gráfico 23
...
Quanto ao nível de endividamento durante a pandemia, `r pct_endividamento[n]` dos estudantes afirmaram que seu nível de endividamento `r tolower(names(casos_endividamento[n]))`, como é possível observar no Gráfico 23.

```{r, grafico-23, echo=FALSE}
#Gráfico 23: Dívidas durante pandemia
# Gráfico tipo "pizza"
pie(casos_endividamento,
    ...
    main = "Gráfico 23: Dívidas durante a pandemia")
```

Já `r pct_endividamento[n-1]` afirmaram que o seu nível de endividamento `r tolower(names(casos_endividamento[n-1]))` e `r pct_endividamento[n-2]` que `r tolower(names(casos_endividamento[n-2]))`. Dos dados compilados neste quesito, apenas  `r pct_endividamento[n-2] não souberam informar, como ilustra o Gráfico 22. 
Pensando em um cenário "pós-pandêmico" (após o pico da pandemia 2020/2022), foi solicitado aos alunos participantes da pesquisa se manifestar em quais dos seus gastos realizados durante a pandemia apontaram crescimento. No questionário foram relacionados para tanto alguns segmentos específicos para escolha do respondente.  

```{r grafico-24-pre-proc, echo=FALSE}
## Gráfico 24 
...
# Colocando nomes com strings menores para caber no gráfico de barras
names(casos_perfil_despesas) <- c("Relac. saúde", "Deslocamento", "Transp. urbano", 
                                  "Aluguel", "Internet", "Alimentação", "Outras")
# colocando as despesas já nomeadas em ordem crescente
casos_perfil_despesas <- sort(casos_perfil_despesas)
# calculando os percentuias já com o símbolo % 
pct_perfil_despesas <- paste0(round(unname(casos_perfil_despesas) / sum(unname(casos_perfil_despesas)) * 100,0), "%")
# definindo o número de ocorrências
n = length(casos_perfil_despesas)
```

Portanto, nesta questão de múltipla escolha os respondentes poderiam indicar mais de um segmento simultaneamente. Os dados compilados das respostas estão ilustrados no Gráfico 24.  

```{r, grafico-24, echo=FALSE}
# Gráfico 23 do tipo barras
graph.perfil.despesas <- barplot(casos_perfil_despesas, 
                       main = "Gráfico 24: Despesas que cresceram na pandemia",
                       xlab = "Despesas", 
                       ylab = "Quantidade",
                       col = "orange",
...
Dentre os gastos citados pelos respondentes pós pandemia e agrupados na figura do Gráfico 24, observa-se que o maior crescimento apontado foi com `r tolower(names(casos_perfil_despesas[n]))` (`r pct_perfil_despesas[n]`) com `r unname (casos_perfil_despesas)[n]` casos, seguido por `r tolower(names(casos_perfil_despesas[n-1]))`, com `r pct_perfil_despesas[n-1]`, e `r tolower(names(casos_perfil_despesas[n-2]))`, com `r pct_perfil_despesas[n-2]`. 
...
Demais despesas apontadas, tais como `r tolower(names(casos_perfil_despesas[n-3]))` responderam com `r pct_perfil_despesas[n-3]`, `r tolower(names(casos_perfil_despesas[n-4]))` resultaram em `r pct_perfil_despesas[n-4]` representaram um impacto menor nos alunos questionados.  

## Análise qualitativa
Questões com gráfico de chuvas de palavras!
Questões com interpretações em aberto !!
Como agregar e analisá-las ? Utilizando-se "análise de conteúdo"?

```{r, grafico-25-pre-proc, echo=FALSE, warning = FALSE, message = FALSE}
# Gráfico 25: situacao_durante-pandemia
# Nuvem de palavras
...
```{r grafico-25, echo=F, warning = FALSE, message = FALSE}
...
mtext("Gráfico 25: Manifestação sobre a sua situação durante a pandemia", side=3, line = 3)
...
```{r, grafico-26-pre-proc, echo=FALSE, warning = FALSE, message = FALSE}
#Gráfico 26: Qualidade de vida
#Carregando o texto e eliminando os NA (missing data)
...
```{r, grafico-26, echo=FALSE, warning = FALSE, message = FALSE}
# Gerar a núvem de palavras
set.seed(1234) # para reprodutibilidade 
wordcloud(words = df$word, freq = df$freq, min.freq = 1, # maior "min.freq", maior/precisão, menor número/palavras     
          max.words=200, random.order=FALSE, rot.per=0.35,       
          colors=brewer.pal(8, "Dark2"))
mtext("Gráfico 26: Manifestação sobre a sua qualidade de vida", side=3, line = 3)
...
```{r, grafico-27-pre-proc, echo=FALSE, warning = FALSE, message = FALSE}
#Gráfico 27: IES fez de positivo na pandemia
#Carregando o texto e eliminando os NA (missing data)
...
```{r, grafico-27, echo=FALSE, warning = FALSE, message = FALSE}
# Gerar a núvem de palavras
set.seed(1234) # para reprodutibilidade 
wordcloud(words = df$word, freq = df$freq, min.freq = 1, # maior "min.freq", maior/precisão, menor número/palavras     
          max.words=200, random.order=FALSE, rot.per=0.35,       
          colors=brewer.pal(8, "Dark2"))
mtext("Gráfico 27: O que a IES fez de positivo na pandemia", side=3, line = 3)
...
```{r, grafico-28-pre-proc, echo=FALSE, warning = FALSE, message = FALSE}
#Gráfico 28: IES poderia melhorar na pandemia
#Carregando o texto e eliminando os NA (missing data)
...
```{r, grafico-28, echo=FALSE, warning = FALSE, message = FALSE}
# Gerar a núvem de palavras
set.seed(1234) # para reprodutibilidade 
wordcloud(words = df$word, freq = df$freq, min.freq = 1, # maior "min.freq", maior/precisão, menor número/palavras     
          max.words=200, random.order=FALSE, rot.per=0.35,       
          colors=brewer.pal(8, "Dark2"))
mtext("Gráfico 28: O que a IES poderia melhorar na pandemia", side=3, line = 3)
...
```{r, grafico-29-pre-proc, echo=FALSE, warning = FALSE, message = FALSE}
#Gráfico 29: IES poderia melhor ajudar na pandemia
#Carregando o texto e eliminando os NA (missing data)
...
```{r, grafico-29, echo=FALSE, warning = FALSE, message = FALSE}
# Gerar a núvem de palavras
set.seed(1234) # para reprodutibilidade 
wordcloud(words = df$word, freq = df$freq, min.freq = 1, # maior "min.freq", maior/precisão, menor número/palavras     
          max.words=200, random.order=FALSE, rot.per=0.35,       
          colors=brewer.pal(8, "Dark2"))
mtext("Gráfico 29: Em que a IES poderia melhor ajudar na pandemia", side=3, line = 3)
...
```{r, grafico-38, echo=FALSE, warning=FALSE, error=FALSE}
...
```{r, grafico-39, echo=FALSE, error=FALSE, warning=FALSE}
...
```{r, grafico-40, echo=FALSE, warning=FALSE, error=FALSE}
...
#Gráfico 41: Acesso aos serviços de saude
# Gráfico tipo "pizza"
pie(casos_acesso_saude,
    ...
    main = "Gráfico 41: Acesso aos serviços de saude")
```
